...
Paladins who swear the Oath of the Ledger are the cosmic debt collectors of the multiverse. They don't care about good or evil—they care about balance, contracts, and the settle-up. They hunt oathbreakers, tax evaders of the soul, and those who try to cheat their destiny. Their armor is often etched with golden numbers that shift on their own, and their smites ring with the sound of dropping coins.
...
\midrule
3rd & \textit{borrowed luck, hellish rebuke} \\
5th & \textit{snake eyes, zone of truth} \\
9th & \textit{russian roulette, slow} \\
13th & \textit{all in, locate creature} \\
17th & \textit{liquidation, geas} \\
\bottomrule
\end{tabularx}
\end{center}
...
The Oath of the Debtor is rarely sworn willingly. It is the path of those who have gambled their lives and lost, only to be given a second chance at a terrible interest rate. These paladins are bound to the Proprietor or similar cosmic entities, serving as living collateral. Their magic feels cold and desperate, often manifesting as spectral chains, coins that rust upon touch, or shadows that bleed.
...
\textbf{Paladin Level} & \textbf{Spells} \\
\midrule
3rd & \textit{borrowed luck, false life} \\
5th & \textit{fold, darkness} \\
9th & \textit{russian roulette, life transference} \\
13th & \textit{all in, shadow of moil} \\
17th & \textit{liquidation, reincarnate} \\
\bottomrule
\end{tabularx}
\end{center}
...
Starting at 7th level, you project an aura of shared risk. The aura extends 10 feet from you in every direction. Whenever you take damage, you can choose to deal necrotic damage equal to your Charisma modifier to a hostile creature within the aura.
...
\begin{tabularx}{\linewidth}{l X}
    ...
    \midrule
    1st & \textit{borrowed luck, command} \\
    2nd & \textit{snake eyes, zone of truth} \\
    3rd & \textit{russian roulette, clairvoyance} \\
    4th & \textit{all in, locate creature} \\
    5th & \textit{liquidation, geas} \\
    \bottomrule
\end{tabularx}
\end{center}
...
At 14th level, you can call in a debt with lethal force. As an action, you touch a creature. The target must make a Charisma saving throw against your warlock spell save DC. On a failed save, the target takes \textbf{55 (10d10) force damage}. On a successful save, it takes half as much damage.
...
\begin{tabularx}{\linewidth}{l X}
    ...
    \midrule
    1st & \textit{borrowed luck, chaos bolt} \\
    3rd & \textit{snake eyes, Nystul's magic aura} \\
    5th & \textit{dead man's volley, blink} \\
    7th & \textit{all in, confusion} \\
    9th & \textit{liquidation, mislead} \\
    \bottomrule
\end{tabularx}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{2 (Clubs):} The target must succeed on a Strength saving throw (DC 8 + your proficiency bonus + your Charisma modifier) or be pushed 10 feet away.
...
At 13th level, you can manipulate positioning on the battlefield. As a bonus action, you can throw a card at a creature within 30 feet. The target must make a Charisma saving throw (DC 8 + your proficiency bonus + your Charisma modifier).
\begin{itemize}
...
At 17th level, you can unleash your entire deck in a storm of blades. As an action, you spray cards in a 30-foot cone. Each creature in that area must make a Dexterity saving throw (DC 8 + your proficiency bonus + your Dexterity modifier).
\begin{itemize}
...
\textit{\textbf{Void Stamp (Recharge 5–6).}} The Croupier attempts to cancel a creature's existence. It slams its stamp onto a prone creature within 5 feet. The target must make a \textbf{DC 15 Constitution saving throw}. On a failure, the target takes 33 (6d10) necrotic damage and is \textbf{stunned} until the end of its next turn. On a success, it takes half damage and isn't \textbf{stunned}.
...
% =========================================================
\chapter{Spells}
The magic of the Proprietor is not woven from the Weave, but borrowed from the ledger of fate. These spells are added to the spell lists of the \textbf{Bard}, \textbf{Cleric}, \textbf{Druid}, \textbf{Sorcerer}, \textbf{Warlock}, and \textbf{Wizard} classes.
...
\textbf{At Higher Levels.} When you cast this spell using a spell slot of 5th level or higher, the necrotic damage you take (and thus the potential bonus damage) increases by \textbf{1d10} for each slot level above 4th.
...
\textbf{Range:} 30 feet \\
\textbf{Components:} V, S, M (a poker chip) \\
\textbf{Duration:} 1 hour

You open a magical pot, inviting your allies to invest their vitality for mutual protection. Up to 5 willing creatures within range (including yourself) can choose to spend one Hit Die.

Roll all the spent Hit Dice and add them together. The total is the \textbf{Pot}. You gain a pool of Temporary Hit Points equal to the Pot + your spellcasting ability modifier. As a bonus action on your turn, you can transfer any amount of these Temporary Hit Points to a creature within 30 feet of you.

\textbf{At Higher Levels.} When you cast this spell using a spell slot of 3rd level or higher, the maximum number of Hit Dice that can be spent increases by \textbf{1} for each slot level above 2nd.

% --- BAD BEAT ---
\subsection{Bad Beat}
\textit{2nd-level enchantment}

\textbf{Casting Time:} 1 reaction, which you take when a creature you can see scores a critical hit against you or an ally \\
\textbf{Range:} 60 feet \\
\textbf{Components:} V \\
\textbf{Duration:} Instantaneous

You shout a curse of disbelief, forcing fate to rewrite the outcome. The critical hit becomes a normal hit.

Additionally, the attacker must succeed on a Wisdom saving throw or take \textbf{2d8 psychic damage} from the shock of their stolen glory.

\textbf{At Higher Levels.} When you cast this spell using a spell slot of 3rd level or higher, the damage increases by \textbf{1d8} for each slot level above 2nd.
...
\textbf{At Higher Levels.} When you cast this spell using a spell slot of 4th level or higher, the initial slashing damage increases by \textbf{1d6} for each slot level above 3rd.
...
\textbf{At Higher Levels.} When you cast this spell using a spell slot of 3rd level or higher, the damage reduction increases by \textbf{1d10} for each slot level above 2nd.
...
\textbf{At Higher Levels.} When you cast this spell using a spell slot of 7th level or higher, the bludgeoning damage increases by \textbf{1d8} and the fire damage increases by \textbf{1d8} for each slot level above 6th.
...
\textbf{At Higher Levels.} When you cast this spell using a spell slot of 6th level or higher, the initial force damage increases by \textbf{1d10}, and the hit point threshold for the execution effect increases by \textbf{10} for each slot level above 5th.

% --- PENNY PINCH ---
\subsection{Penny Pinch}
...
You make a pinching motion towards a creature, attempting to tax their very life force. The target must succeed on a Constitution saving throw or take \textbf{1d6 necrotic damage}.

If the target fails the save, you gain 1 temporary hit point. This spell's damage increases by 1d6 when you reach 5th level (2d6), 11th level (3d6), and 17th level (4d6).

% --- RUSSIAN ROULETTE ---
\subsection{Russian Roulette}
\textit{3rd-level necromancy}

\textbf{Casting Time:} 1 action \\
\textbf{Range:} 60 feet \\
\textbf{Components:} V, S, M (a spinning top or cylinder) \\
\textbf{Duration:} Instantaneous

You summon a spectral revolver with a single bullet in the chamber. You aim it at a creature within range and let fate decide the outcome. Roll a \textbf{d6}.

\begin{itemize}
    \item \textbf{1 (The Backfire):} The mechanism jams catastrophically. The spell fails, and you take \textbf{3d10 necrotic damage}. The target takes no damage.
    \item \textbf{2-5 (The Click):} The chamber is empty, but the sound of the hammer snapping snaps the target's nerve. The target takes \textbf{3d10 psychic damage}.
    \item \textbf{6 (The Shot):} The bullet fires with thunderous force. The target takes \textbf{10d10 necrotic damage}.
\end{itemize}

\textbf{At Higher Levels.} When you cast this spell using a spell slot of 4th level or higher, the damage for "The Shot" increases by \textbf{2d10} for each slot level above 3rd.
...
\textbf{At Higher Levels.} When you cast this spell using a spell slot of 3rd level or higher, the necrotic damage increases by \textbf{1d6} for each slot level above 2nd.

% --- WHEEL OF FATE ---
\subsection{Wheel of Fate}
\textit{7th-level evocation}

\textbf{Casting Time:} 1 action \\
\textbf{Range:} 120 feet \\
\textbf{Components:} V, S, M (a wheel or spinner) \\
\textbf{Duration:} Instantaneous

You summon a massive, spectral wheel of fortune that appears in the sky and spins wildly. Choose a point within range. All creatures within a 30-foot-radius sphere centered on that point must make a Constitution saving throw.

Roll a \textbf{d8} to determine the effect. Creatures take \textbf{10d8 damage} of the type corresponding to the \textbf{d8 result table} below on a failed save, or half as much on a successful one.

\begin{enumerate}
    \item \textbf{Red:} Fire.
    \item \textbf{Blue:} Cold.
    \item \textbf{Green:} Acid.
    \item \textbf{Yellow:} Lightning.
    \item \textbf{White:} Force.
    \item \textbf{Black:} Necrotic.
    \item \textbf{Gold (The House Wins):} Psychic. Additionally, you regain hit points equal to half the total damage dealt.
    \item \textbf{Jackpot:} Roll twice on this table and apply both effects. If you roll an 8 on either of the new rolls, reroll that die until a different number appears.
\end{enumerate}

\textbf{At Higher Levels.} When you cast this spell using a spell slot of 8th level or higher, the damage increases by \textbf{1d8} for each slot level above 7th.
